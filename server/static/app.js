...
`;var ao=new Set(["file","-file","path","-path","repo","-repo","tags","-tags","case","lit","max_matches"]);function wt(t){let e={},r="",o="",s="",i=t.trim(),n=/\[|\(|(?:^([a-zA-Z0-9\-_]+):|\\.)| /,d=!0;for(;i.length>0;){let u=n.exec(i);if(u===null){s+=i;break}s+=i.slice(0,u.index);let f=u[0];if(i=i.slice(u.index+f.length),d=d&&u.index===0,f===" ")o===""?s+=" ":(_t(e,o,s),o="",s="");else if(f==="("||f==="["){let p=f==="("?")":"]",E=1,b=0,y=!1,F="";for(;b<i.length&&E>0;){let W=i[b];b++,y?y=!1:W==="\\"?y=!0:W===f?E++:W===p&&E--,F+=W}s+=f+F,i=i.slice(b)}else if(f.startsWith("\\"))s+=f;else{let p=u[1];o===""&&p&&ao.has(p)?(s.trim()!==""&&_t(e,o,s),s="",o=p):s+=f}d=f===" "}_t(e,o,s);let l=e[""]||[];return delete e[""],r=l.join(" ").trim(),{line:r,operators:e}}function _t(t,e,r){t[e]||(t[e]=[]),t[e].push(r)}function ar(t,e={},r={}){let o=new URLSearchParams;t.trim()&&o.set("q",t.trim()),e.literal&&o.set("literal","true"),e.caseSensitive&&o.set("fold_case","false");for(let[s,i]of Object.entries(r))for(let n of i)o.append(s,n);return o}async function lr(t,e,r){let o="/api/search?"+t.toString(),s=await fetch(o,{signal:r});if(!s.ok){let i=await s.text();throw new Error(`search failed (${s.status}): ${i}`)}if(!s.body)throw new Error("response has no body");await lo(s.body,i=>{switch(i.type){case"result":e.onResult?.(i);break;case"file":e.onFile?.(i);break;case"facets":e.onFacets?.(i);break;case"done":e.onDone?.(i);break}})}async function lo(t,e){let r=t.getReader(),o=new TextDecoder,s="";try{for(;;){let{done:n,value:d}=await r.read();if(n)break;s+=o.decode(d,{stream:!0});let l;for(;(l=s.indexOf(`
`))!==-1;){let u=s.slice(0,l).trim();s=s.slice(l+1),u.length!==0&&e(JSON.parse(u))}}s+=o.decode();let i=s.trim();i.length>0&&e(JSON.parse(i))}finally{r.releaseLock()}}function cr(t){let e=t.indexOf("/+/");if(e===-1)return{repo:t,version:"",filePath:""};let r=t.slice(0,e),o=t.slice(e+3),s=r.indexOf("/");return s===-1?{repo:r,version:"",filePath:o}:{repo:r.slice(0,s),version:r.slice(s+1),filePath:o}}var ce=ne(()=>U.get().params.get("q")??""),Ys=ne(()=>{let t=U.get().params;return{literal:t.get("literal")==="true",caseSensitive:t.get("fold_case")==="false"}}),dr=ne(()=>{let t=U.get().params.get("context");if(t!==null){let e=parseInt(t,10);if(!isNaN(e)&&e>=0)return e}return 3}),ei=ne(()=>wt(ce.get())),we=N([]),Ze=N([]),Se=N(null),Ee=N(null),Z=N(!1),le=N(null),Et=N(!1),pr=ne(()=>{let t=Ze.get();return Et.get()||t.length<=10?t:t.slice(0,10)}),St=null;async function ur(){St&&St.abort();let t=new AbortController;St=t;let e=ce.get();if(!e){we.set([]),Ze.set([]),Se.set(null),Ee.set(null),Z.set(!1),le.set(null),Et.set(!1);return}let r=U.get(),o=new URLSearchParams(r.params),s=wt(e);Et.set(s.line===""),Z.set(!0),le.set(null),we.set([]),Ze.set([]),Se.set(null),Ee.set(null);let i=[],n=[],d=null;try{await lr(o,{onResult(l){i.push(l)},onFile(l){n.push(l)},onFacets(l){d=l},onDone(l){we.set(i),Ze.set(n),Se.set(d),Ee.set(l),Z.set(!1)},onError(l){t.signal.aborted||(le.set(l.message),Z.set(!1))}},t.signal)}catch(l){t.signal.aborted||(le.set(l instanceof Error?l.message:String(l)),Z.set(!1))}}var Je=null,_e="";function kt(t,e={},r={}){Je&&clearTimeout(Je);let o=ar(t,e,r),s=t!==_e,i=t?`${t} \xB7 code search`:"code search";document.title=i;let n=o.toString(),d="/search"+(n?"?"+n:"");s&&_e!==""?(history.pushState(null,i,d),_e=t):(history.replaceState(null,i,d),_e===""&&(_e=t)),U.set({name:"search",params:o}),Je=setTimeout(()=>{Je=null,ur()},100)}{let t=ce.get();t&&(document.title=`${t} \xB7 code search`,ur())}var j=class extends v{constructor(){super(...arguments);this.value="";this.debounce=100;this.error="";this.timer=null}render(){return c`
    ...
    `):""}setCase(r){let o=r==="false";this.options={...this.options,caseSensitive:o},this.fireChange()}toggleLiteral(){this.options={...this.options,literal:!this.options.literal},this.fireChange()}fireChange(){this.dispatchEvent(new CustomEvent("options-change",{detail:this.options,bubbles:!0,composed:!0}))}};X.styles=[ir,ae,m`
      :host {
        display: block;
      }
    ...
    `,a([h()],T.prototype,"text",2),a([h()],T.prototype,"href",2),a([h({type:Number})],T.prototype,"start",2),a([h({type:Number})],T.prototype,"end",2),a([h()],T.prototype,"repo",2),a([h()],T.prototype,"version",2),T=a([x("filename-match")],T);var P=class extends v{render(){let e=this.start!=null&&this.end!=null;return c`<a class=${At({"lno-link":!0,matchlno:e})} href="${this.href}"
    ...
    `,a([h({type:Number})],P.prototype,"lineNo",2),a([h()],P.prototype,"text",2),a([h()],P.prototype,"href",2),a([h({type:Number})],P.prototype,"start",2),a([h({type:Number})],P.prototype,"end",2),P=a([x("match-line")],P);function co(t){let e=t.lastIndexOf("/");return e<0?".":t.slice(0,e)}function po(t){let e=t.lastIndexOf("/");return e<0?t:t.slice(e+1)}var Y=class extends v{constructor(){super(...arguments);this.noContext=!1}render(){let{repo:r,version:o,filePath:s}=cr(this.result.path),i=`/view/${this.result.path}`,n=this.splitGroups(this.result.lines),d=o.length>6?o.slice(0,6):o,l=co(s),u=po(s);return c`
      <div class="file-group">
        ...
        ${n.map(f=>c`
            <div class="match">
              <div class="contents">
                ${f.map(p=>{let E=p[0],b=p[1],y=p.length>2?p[2]:void 0,F=y!==void 0&&y.length>0,W=`${i}#L${E}`,Xe=F&&y?y[0][0]:void 0,Ye=F&&y?y[0][1]:void 0;return c`
                    <match-line
                      .lineNo=${E}
                      text=${b}
                      href=${W}
    ...
    `}splitGroups(r){let o=[],s=[];for(let i of r)i===null?s.length>0&&(o.push(s),s=[]):s.push(i);return s.length>0&&o.push(s),o}};Y.styles=[sr,H,m`
      :host {
        display: block;
      }
      ...
      <div id="resultbox">
        ${f?c`
          <div id="resultarea">
            ...
            >
              <div id="path-results">
                ${s.map(p=>{let{repo:E,version:b,filePath:y}=uo(p.path);return c`
                    <filename-match
                      text=${y}
                      start=${p.match[0]}
                      end=${p.match[1]}
