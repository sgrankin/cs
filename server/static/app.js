...
`))!==-1;){let h=s.slice(0,l).trim();s=s.slice(l+1),h.length!==0&&e(JSON.parse(h))}}s+=o.decode();let i=s.trim();i.length>0&&e(JSON.parse(i))}finally{t.releaseLock()}}function de(r){let e=r.indexOf("/+/");if(e===-1)return{repo:r,version:"",filePath:""};let t=r.slice(0,e),o=r.slice(e+3),s=t.indexOf("/");return s===-1?{repo:t,version:"",filePath:o}:{repo:t.slice(0,s),version:t.slice(s+1),filePath:o}}var tt=class{constructor(){this.lastCommittedUrl=""}commit(e,t={},o={}){let s=ur(e,t,o),i=e?`${e} \xB7 code search`:"code search",n=et(s);if(!e)return this.lastCommittedUrl="",[{type:"replaceUrl",url:et(new URLSearchParams),title:i},{type:"clearResults"}];let c=[];return n!==this.lastCommittedUrl&&this.lastCommittedUrl!==""?c.push({type:"pushUrl",url:n,title:i}):c.push({type:"replaceUrl",url:n,title:i}),c.push({type:"search",params:s}),this.lastCommittedUrl=n,c}popstate(e,t){this.lastCommittedUrl=et(t);let o=e?`${e} \xB7 code search`:"code search",s=[{type:"replaceUrl",url:et(t),title:o}];return e?s.push({type:"search",params:t}):s.push({type:"clearResults"}),s}};function et(r){let e=r.toString();return"/search"+(e?"?"+e:"")}var fe=le(()=>O.get().params.get("q")??""),ui=le(()=>{let r=O.get().params;return{literal:r.get("literal")==="true",caseSensitive:r.get("fold_case")==="false"}}),mr=le(()=>{let r=O.get().params.get("context");if(r!==null){let e=parseInt(r,10);if(!isNaN(e)&&e>=0)return e}return 3}),fi=le(()=>Pt(fe.get())),pe=L([]),Ae=L([]),he=L(null),ue=L(null),B=L(!1),X=L(null),rt=L(!1),gr=le(()=>{let r=Ae.get();return rt.get()||r.length<=10?r:r.slice(0,10)}),Tt=null;async function mo(){Tt&&Tt.abort();let r=new AbortController;Tt=r;let e=fe.get();if(!e){pe.set([]),Ae.set([]),he.set(null),ue.set(null),B.set(!1),X.set(null),rt.set(!1);return}let t=O.get(),o=new URLSearchParams(t.params),s=Pt(e);rt.set(s.line===""),B.set(!0),X.set(null),pe.set([]),Ae.set([]),he.set(null),ue.set(null);let i=[],n=[],c=null;try{await fr(o,{onResult(l){i.push(l)},onFile(l){n.push(l)},onFacets(l){c=l},onDone(l){pe.set(i),Ae.set(n),he.set(c),ue.set(l),B.set(!1)},onError(l){r.signal.aborted||(X.set(l.message),B.set(!1))}},r.signal)}catch(l){r.signal.aborted||(X.set(l instanceof Error?l.message:String(l)),B.set(!1))}}var ot=new tt,Y=null;function vr(r,e={},t={}){Y&&clearTimeout(Y),Y=setTimeout(()=>{Y=null,it(ot.commit(r,e,t))},200)}function st(r,e={},t={}){Y&&(clearTimeout(Y),Y=null),it(ot.commit(r,e,t))}{let e=O.get().params.get("q")??"";e&&it(ot.commit(e))}window.addEventListener("popstate",()=>{let r=O.get(),e=r.params.get("q")??"";it(ot.popstate(e,r.params))});function it(r){for(let e of r)switch(e.type){case"pushUrl":history.pushState(null,e.title,e.url),document.title=e.title,O.set({name:"search",params:new URLSearchParams(new URL(e.url,location.origin).search)});break;case"replaceUrl":history.replaceState(null,e.title,e.url),document.title=e.title,O.set({name:"search",params:new URLSearchParams(new URL(e.url,location.origin).search)});break;case"search":mo();break;case"clearResults":pe.set([]),Ae.set([]),he.set(null),ue.set(null),B.set(!1),X.set(null),rt.set(!1);break}}var V=class extends g{constructor(){super(...arguments);this.value="";this.error=""}render(){return d`
    ...
    `}setCase(t){let o=t==="false";this.options={...this.options,caseSensitive:o},this.fireChange()}toggleLiteral(){this.options={...this.options,literal:!this.options.literal},this.fireChange()}fireChange(){this.dispatchEvent(new CustomEvent("options-change",{detail:this.options,bubbles:!0,composed:!0}))}};ee.styles=[pr,ce,m`
      :host {
        display: block;
      }
    ...
    `,a([f({type:Number})],M.prototype,"lineNo",2),a([f()],M.prototype,"text",2),a([f()],M.prototype,"href",2),a([f({type:Number})],M.prototype,"start",2),a([f({type:Number})],M.prototype,"end",2),M=a([x("match-line")],M);function go(r){let e=r.lastIndexOf("/");return e<0?".":r.slice(0,e)}function vo(r){let e=r.lastIndexOf("/");return e<0?r:r.slice(e+1)}var te=class extends g{constructor(){super(...arguments);this.noContext=!1}render(){let{repo:t,version:o,filePath:s}=de(this.result.path),i=`/view/${this.result.path}`,n=this.splitGroups(this.result.lines),c=o.length>6?o.slice(0,6):o,l=go(s),h=vo(s);return d`
      <div class="file-group">
        ...
        ${n.map(u=>d`
            <div class="match">
              <div class="contents">
                ${u.map(p=>{let E=p[0],b=p[1],y=p.length>2?p[2]:void 0,H=y!==void 0&&y.length>0,j=`${i}#L${E}`,nt=H&&y?y[0][0]:void 0,at=H&&y?y[0][1]:void 0;return d`
                    <match-line
    ...
    `}splitGroups(t){let o=[],s=[];for(let i of t)i===null?s.length>0&&(o.push(s),s=[]):s.push(i);return s.length>0&&o.push(s),o}};te.styles=[dr,U,m`
      :host {
        display: block;
      }
    ...
    `],a([f({type:Number})],W.prototype,"total",2),a([f({type:Number})],W.prototype,"timeMs",2),a([f({type:Boolean})],W.prototype,"truncated",2),a([f({type:Boolean})],W.prototype,"loading",2),W=a([x("cs-result-stats")],W);var re=class extends g{constructor(){super(...arguments);this.facets=null;this.selected={}}render(){let t=this.facets&&(this.facets.ext?.length||this.facets.repo?.length||this.facets.path?.length),o=Object.values(this.selected).some(n=>n.size>0);if(!t&&!o)return S;let i=[{label:"Extension",key:"f.ext",buckets:this.facets?.ext??[]},{label:"Repository",key:"f.repo",buckets:this.facets?.repo??[]},{label:"Path",key:"f.path",buckets:this.facets?.path??[]}].filter(n=>n.buckets.length>0||(this.selected[n.key]?.size??0)>0);return i.length===0?S:d`
      <div class="panel">
        ${i.map(n=>this.renderSection(n.label,n.key,n.buckets))}
      </div>
    `}renderSection(t,o,s){let i=this.selected[o]??new Set,c=[...s].sort((u,p)=>p.c-u.c||u.v.localeCompare(p.v)).slice(0,10),l=new Set(c.map(u=>u.v)),h=[...i].filter(u=>!l.has(u));return d`
      <div class="section">
        ...
        ${c.map(u=>d`
          <button
            class=${i.has(u.v)?"pill active":"pill"}
  ...
  `,a([f()],me.prototype,"path",2),me=a([x("cs-breadcrumbs")],me);var oe=class extends g{constructor(){super(...arguments);this.entries=[];this.basePath=""}render(){let t=[...this.entries].sort((o,s)=>{let i=o.endsWith("/"),n=s.endsWith("/");return i!==n?i?-1:1:o.localeCompare(s)});return d`
      <div class="listing">
...
`);return t.length>0&&t[t.length-1]===""&&t.pop(),d`
    ...
    `}onLineClick(t,o){if(t.shiftKey&&this.selectedStart>0){t.preventDefault();let s=Math.min(this.selectedStart,o),i=Math.max(this.selectedStart,o);this.selectedStart=s,this.selectedEnd=i,history.replaceState(null,"",`#L${s}-L${i}`)}else this.selectedStart=o,this.selectedEnd=o}};T.styles=m`
    .selection-hint {
      font-size: 11px;
      color: var(--color-foreground-subtle);
      padding: 4px 8px;
