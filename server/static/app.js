...
`))!==-1;){let c=s.slice(0,a).trim();s=s.slice(a+1),c.length!==0&&t(JSON.parse(c))}}s+=i.decode();let o=s.trim();o.length>0&&t(JSON.parse(o))}finally{e.releaseLock()}}function ut(r){let t=r.indexOf("/+/");if(t===-1)return{repo:r,version:"",filePath:""};let e=r.slice(0,t),i=r.slice(t+3),s=e.lastIndexOf("/");return s===-1?{repo:e,version:"",filePath:i}:{repo:e.slice(0,s),version:e.slice(s+1),filePath:i}}function Kr(r,t={},e={}){let i=new URLSearchParams;if(r.trim()&&i.set("q",r.trim()),t.literal&&i.set("literal","true"),t.caseSensitive&&i.set("fold_case","false"),t.repos?.length)for(let s of t.repos)i.append("repo",s);for(let[s,o]of Object.entries(e))for(let n of o)i.append(s,n);return i}var ge=class{constructor(){this.lastCommittedUrl=""}commit(t,e={},i={}){let s=Kr(t,e,i),o=t?`${t} \xB7 code search`:"code search",n=me(s);if(!t)return this.lastCommittedUrl="",[{type:"replaceUrl",url:me(new URLSearchParams),title:o},{type:"clearResults"}];let l=[];return n!==this.lastCommittedUrl&&this.lastCommittedUrl!==""?l.push({type:"pushUrl",url:n,title:o}):l.push({type:"replaceUrl",url:n,title:o}),l.push({type:"search",params:s}),this.lastCommittedUrl=n,l}popstate(t,e){this.lastCommittedUrl=me(e);let i=t?`${t} \xB7 code search`:"code search",s=[{type:"replaceUrl",url:me(e),title:i}];return t?s.push({type:"search",params:e}):s.push({type:"clearResults"}),s}};function me(r){let t=r.toString();return"/search"+(t?"?"+t:"")}var _e=class{constructor(){this.results=[];this.files=[];this.dirty=!1;this.flushed=!1}start(){return this.results=[],this.files=[],this.dirty=!1,this.flushed=!1,{loading:!0,error:null,done:null}}onResult(t){this.results.push(t),this.dirty=!0}onFile(t){this.files.push(t),this.dirty=!0}flush(){if(!this.dirty)return null;this.dirty=!1;let t={results:[...this.results],files:[...this.files]};return this.flushed||(t.facets=null,this.flushed=!0),t}onFacets(t){return{facets:t}}onDone(t){return{results:this.results,files:this.files,done:t,loading:!1}}onError(t){return{error:t,loading:!1}}};var ft=le(()=>I.get().params.get("q")??""),Hn=le(()=>{let r=I.get().params;return{literal:r.get("literal")==="true",caseSensitive:r.get("fold_case")==="false"}}),Gr=le(()=>{let r=I.get().params.get("context");if(r!==null){let t=parseInt(r,10);if(!isNaN(t)&&t>=0)return t}return 3}),Mt=j([]),Tt=j([]),It=j(null),Lt=j(null),Ot=j(!1),Nt=j(null),Je=null;async function rs(){Je&&Je.abort();let r=new AbortController;if(Je=r,!ft.get()){Mt.set([]),Tt.set([]),It.set(null),Lt.set(null),Ot.set(!1),Nt.set(null);return}let e=I.get(),i=new URLSearchParams(e.params),s=new _e;Rt(s.start());let o=setInterval(()=>{let n=s.flush();n&&Rt(n)},100);try{await Fr(i,{onResult(n){s.onResult(n)},onFile(n){s.onFile(n)},onFacets(n){Rt(s.onFacets(n))},onDone(n){clearInterval(o),Rt(s.onDone(n))}},r.signal)}catch(n){clearInterval(o),r.signal.aborted||Rt(s.onError(n instanceof Error?n.message:String(n)))}}function Rt(r){"results"in r&&Mt.set(r.results),"files"in r&&Tt.set(r.files),"facets"in r&&It.set(r.facets),"done"in r&&Lt.set(r.done),"loading"in r&&Ot.set(r.loading),"error"in r&&Nt.set(r.error)}var ve=new ge,et=null;function Jr(r,t={},e={}){et&&clearTimeout(et),et=setTimeout(()=>{et=null,xe(ve.commit(r,t,e))},200)}function be(r,t={},e={}){et&&(clearTimeout(et),et=null),xe(ve.commit(r,t,e))}function is(){let r=ft.get();r&&xe(ve.commit(r))}is();window.addEventListener("popstate",()=>{let r=I.get(),t=r.params.get("q")??"";xe(ve.popstate(t,r.params))});function xe(r){for(let t of r)switch(t.type){case"pushUrl":history.pushState(null,t.title,t.url),document.title=t.title,I.set({name:"search",params:new URLSearchParams(new URL(t.url,location.origin).search)});break;case"replaceUrl":history.replaceState(null,t.title,t.url),document.title=t.title,I.set({name:"search",params:new URLSearchParams(new URL(t.url,location.origin).search)});break;case"search":rs();break;case"clearResults":Mt.set([]),Tt.set([]),It.set(null),Lt.set(null),Ot.set(!1),Nt.set(null);break}}var rt=class extends _{constructor(){super(...arguments);this.value="";this.error=""}render(){return p`
    ...
    `,d([m({type:Number})],U.prototype,"lineNo",2),d([m()],U.prototype,"text",2),d([m()],U.prototype,"href",2),d([m({type:Number})],U.prototype,"start",2),d([m({type:Number})],U.prototype,"end",2),U=d([y("match-line")],U);function ss(r){let t=r.lastIndexOf("/");return t<0?".":r.slice(0,t)}function os(r){let t=r.lastIndexOf("/");return t<0?r:r.slice(t+1)}var st=class extends _{constructor(){super(...arguments);this.noContext=!1}render(){let{repo:e,version:i,filePath:s}=ut(this.result.path),o=`/view/${this.result.path}`,n=this.splitGroups(this.result.lines),l=i.length>6?i.slice(0,6):i,a=ss(s),c=os(s);return p`
      <div class="file-group">
        ...
        ${n.map(u=>p`
            <div class="match">
              <div class="contents">
                ${u.map(h=>{let x=h[0],f=h[1],v=h.length>2?h[2]:void 0,k=v!==void 0&&v.length>0,W=`${o}#L${x}`,R=k&&v?v[0][0]:void 0,ye=k&&v?v[0][1]:void 0;return p`
                    <match-line
    ...
    `}splitGroups(e){let i=[],s=[];for(let o of e)o===null?s.length>0&&(i.push(s),s=[]):s.push(o);return s.length>0&&i.push(s),i}};st.styles=[Rr,N,g`
      :host {
        display: block;
      }
  ...
  `,d([m({type:Object})],ot.prototype,"facets",2),d([m({type:Object})],ot.prototype,"selected",2),ot=d([y("cs-facet-panel")],ot);var Dt=class extends _{render(){return p`
    ...
    `],Dt=d([y("cs-search-help")],Dt);function ns(r,t,e){let i=r[t]??new Set,s;return t==="f.path"?s=i.has(e)?new Set:new Set([e]):(s=new Set(i),s.has(e)?s.delete(e):s.add(e)),{...r,[t]:s}}function as(r){let t={};for(let[e,i]of Object.entries(r))i.size>0&&(t[e]=[...i]);return t}function ls(r){let t={};for(let e of["f.ext","f.repo","f.path"]){let i=r.getAll(e);i.length>0&&(t[e]=new Set(i))}return t}var Ut=class extends Et(_){constructor(){super(...arguments);this.currentOptions={}}get activeFacets(){return ls(I.get().params)}render(){let e=ft.get(),i=Mt.get(),s=Tt.get(),o=Lt.get(),n=Ot.get(),l=Nt.get(),a=It.get(),c=Gr.get(),u=o||n;return p`
      <div id="searcharea">
        <form
          autocapitalize="off"
          autocomplete="off"
    ...
    `}onFacetToggle(e){let i=ns(this.activeFacets,e.detail.key,e.detail.value),s=ft.get();s&&be(s,this.currentOptions,this.facetParamsFrom(i))}onSearchInput(e){Jr(e.detail.value,this.currentOptions,this.facetParamsFrom(this.activeFacets))}onSearchSubmit(e){be(e.detail.value,this.currentOptions,this.facetParamsFrom(this.activeFacets))}onOptionsChange(e){this.currentOptions=e.detail,this.reSearch()}reSearch(){let e=ft.get();e&&be(e,this.currentOptions,this.facetParamsFrom(this.activeFacets))}getRepos(){return window.__CS_INIT?.repos??[]}facetParamsFrom(e){return as(e)}};Ut.styles=[N,pt,g`
      :host {
        display: block;
        line-height: normal;
        font-size: 13px;
  ...
  `,d([m({type:Array})],nt.prototype,"entries",2),d([m()],nt.prototype,"basePath",2),nt=d([y("cs-dir-listing")],nt);var cs={navigate(r){window.location.href=r}};function hs(r,t,e){switch(r){case"Enter":return t?{type:"open-tab",url:`/search?q=${encodeURIComponent(t)}`}:{type:"close-help"};case"/":return{type:"navigate",url:`/search${t?"?q="+encodeURIComponent(t):""}`};case"?":return{type:"toggle-help"};case"Escape":return{type:"close-help"};case"v":return e?{type:"navigate",url:e}:{type:"close-help"};case"n":return t?{type:"find",text:t,backwards:!1}:{type:"close-help"};case"p":return t?{type:"find",text:t,backwards:!0}:{type:"close-help"}}return null}function ds(r,t,e,i){return!t||e<=0?Math.floor(r/3):e<=r?.5*(r-e):i/2}function ps(r,t){return r<0?"1":r===t?String(r):`${r}-L${t}`}function us(r,t,e){return r?r.replace("{lno}",ps(t,e)):""}function fs(r){let t=r.match(/^#L(\d+)(?:-L?(\d+))?$/);if(!t)return[-1,-1];let e=parseInt(t[1],10),i=t[2]?parseInt(t[2],10):e;return[e,i]}var L=class extends _{constructor(){super(...arguments);this.content="";this.basePath="";this.repo="";this.version="";this.externalUrl="";this.selectedStart=-1;this.selectedEnd=-1;this.hasSelection=!1;this.onHashChange=()=>{this.parseHash(),this.scrollToSelection()};this.onSelectionChange=()=>{this.hasSelection=(window.getSelection()?.toString()||"").length>0};this.onKeyDown=e=>{e.target.matches("input,textarea")||e.altKey||e.ctrlKey||e.metaKey||this.processKey(e.key)&&e.preventDefault()}}connectedCallback(){super.connectedCallback(),this.parseHash(),window.addEventListener("hashchange",this.onHashChange),document.addEventListener("keydown",this.onKeyDown),document.addEventListener("selectionchange",this.onSelectionChange)}disconnectedCallback(){super.disconnectedCallback(),window.removeEventListener("hashchange",this.onHashChange),document.removeEventListener("keydown",this.onKeyDown),document.removeEventListener("selectionchange",this.onSelectionChange)}parseHash(){let[e,i]=fs(window.location.hash);this.selectedStart=e,this.selectedEnd=i}scrollToSelection(){this.selectedStart<0||this.updateComplete.then(()=>{let e=this.renderRoot.querySelector(`#L${this.selectedStart}`);if(!e)return;let i=this.selectedStart!==this.selectedEnd,s=0;if(i){let l=this.renderRoot.querySelector(`#L${this.selectedEnd}`);if(l){let a=e.getBoundingClientRect(),c=l.getBoundingClientRect();s=c.top+c.height-a.top}}let o=ds(window.innerHeight,i,s,e.offsetHeight),n=e.getBoundingClientRect();window.scrollTo(0,n.top+window.scrollY-o)})}firstUpdated(){this.scrollToSelection()}resolvedExternalUrl(){return us(this.externalUrl,this.selectedStart,this.selectedEnd)}getSelectedText(){return window.getSelection()?.toString()||""}processKey(e){let i=hs(e,this.getSelectedText(),this.resolvedExternalUrl());if(!i)return!1;switch(i.type){case"navigate":cs.navigate(i.url);break;case"open-tab":window.open(i.url);break;case"find":window.find(i.text,!1,i.backwards);break;case"toggle-help":this.dispatchEvent(new CustomEvent("toggle-help",{bubbles:!0,composed:!0}));break;case"close-help":this.dispatchEvent(new CustomEvent("close-help",{bubbles:!0,composed:!0}));break}return!0}render(){let e=this.content.split(`
`);return e.length>0&&e[e.length-1]===""&&e.pop(),p`
  ...
  `,d([m({type:Boolean,reflect:!0})],gt.prototype,"open",2),gt=d([y("cs-help-modal")],gt);function ms(r){let t=r.indexOf("/+/");if(t>=0){let e=r.slice(0,t),i=r.slice(t+3),s=e.lastIndexOf("@");if(s>=0)return{repo:e.slice(0,s),version:e.slice(s+1),filePath:i}}return ut(r)}function gs(r){return r.endsWith("/")||r.endsWith("/+/")||!r.includes("/+/")}function _s(r,t){return`/raw/${r}${t&&!r.endsWith("/")?"/":""}`}function vs(r,t,e){let i="github.com/";return r.startsWith(i)?`https://github.com/${r.slice(i.length)}/blob/${t}/${e}#L{lno}`:""}var P=class extends _{constructor(){super(...arguments);this.path="";this.content=null;this.dirEntries=null;this.readmeContent=null;this.loading=!0;this.error=null;this.showHelp=!1}willUpdate(e){e.has("path")&&this.fetchContent()}async fetchContent(){this.loading=!0,this.error=null,this.content=null,this.dirEntries=null,this.readmeContent=null;let e=gs(this.path),i=_s(this.path,e);try{let s=await fetch(i);if(!s.ok){this.error=`Not found (${s.status})`,this.loading=!1;return}(s.headers.get("Content-Type")??"").includes("application/json")?(this.dirEntries=await s.json(),this.fetchReadme(i)):this.content=await s.text()}catch(s){this.error=s instanceof Error?s.message:String(s)}this.loading=!1}async fetchReadme(e){if(!this.dirEntries)return;let i=this.dirEntries.find(s=>P.README_RE.test(s));if(i)try{let s=e.endsWith("/")?e:e+"/",o=await fetch(s+i);o.ok&&(this.readmeContent=await o.text())}catch{}}render(){let e=ms(this.path),i=e.repo,s=e.version,o=vs(e.repo,e.version,e.filePath);return p`
      <div class="file-view">
  ...
  `,d([m()],P.prototype,"path",2),d([z()],P.prototype,"content",2),d([z()],P.prototype,"dirEntries",2),d([z()],P.prototype,"readmeContent",2),d([z()],P.prototype,"loading",2),d([z()],P.prototype,"error",2),d([z()],P.prototype,"showHelp",2),P=d([y("cs-file-view")],P);var Wt=class extends _{render(){return p`
      <div class="about">
        <p>
    ...
    `],Wt=d([y("cs-about-view")],Wt);var Vt=class extends Et(_){render(){let t=I.get();return p`
    ...
    `}renderView(t){switch(t.name){case"search":return p`<cs-search-view></cs-search-view>`;case"view":return p`<cs-file-view .path=${t.path??""}></cs-file-view>`;case"about":return p`<cs-about-view></cs-about-view>`;default:return p`<div class="placeholder">Not found</div>`}}};Vt.styles=[N,g`
      :host {
        display: block;
      }
    ...
    `],Vt=d([y("cs-app")],Vt);export{Vt as CsApp};
